...
def identify_mea_passing_genes(trait, geneset_input, FDR_threshold, percentile_threshold, network, input_path ):
    ...
    summary_filepath = os.path.join(input_path,"summary_alternate",f"{trait}_{network}_summary.csv")
    ...
    for threshold in thresholds: 
        ...
        for fdr in fdr_thresholds:
            fishnet_df = pd.DataFrame(columns = ["Threshold", "Network", "numNominal", "Trait", "NumFISHNETGenes", "FISHNETGenes"])
            temp_summary_df = summary_df[summary_df["Ranks"] <= threshold]
            ...
            mea_passing_genes = pd.read_csv(os.path.join(input_path,"results","raw_alternate",f"{network}_{trait}_{network}_or_fishnet_genes.csv"))
            ...
            fishnet_df.to_csv(os.path.join(input_path,"summary_alternate",f"{network}_{trait}_fishnet_genes_{fdr}.csv"), index = None)
